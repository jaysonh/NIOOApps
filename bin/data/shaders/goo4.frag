...
const vec3 Color = vec3(0.085, 0.98, 0.5);
...
float fbm(vec3 p) {
    ...
    f += 0.2500 * noise(p);
    p *= 2.02 ;
    //f += 0.1250 * noise(p)* soundScale*10.0;
    f += 0.1250 * noise(p)* sin(soundCount*0.01);
...
vec2 path(float z) {
    return vec2(TunnelAmpA * sin(z * TunnelFreqA)  , TunnelAmpB * cos(z * TunnelFreqB+ (iTime*0.01 ) + soundCount*0.01));
}

float noiseDist(vec3 p) {
    p = p / NoiseScale * (0.5+abs(sin(iTime*0.01 )));
...
vec3 render(vec3 ro, vec3 rd) {
    ...
    if (m > -0.5) {
        ...
        // material
        col = Color + sin(t * 0.8) * 0.93;
        col += 0.3 * sin(vec3(0.915, 0.82, 0.10) * 0.75 * 16.0);
        ...
        nor = normalize(nor - normalize(pos) * 0.02);
        ref = reflect(rd, nor);
        spe = pow(clamp(dot(ref, lig), 0.0, 1.0), 10.0);
        brdf += 2.20 * spe * vec3(1.00, 0.90, 0.60) * dif;
        
        brdf += 0.40 * amb * vec3(0.50, 0.70, 1.00) * occ;
        brdf += 0.40 * fre * vec3(1.00, 0.00, 1.00) * occ;
...
void main(void) {
    ...
    float t = 0.0;//iTime * CameraSpeed + 4.0 * 60.0;
    vec3 ro = vec3(path(t), t);
    
    t += 0.5;
    vec3 target = vec3(path(t), t);
    vec3 dir = normalize(target - ro);
    vec3 up = vec3(-0.9309864, -0.33987653, 0.1332234) * rotationZ(0.0);//* rotationZ(iTime * 0.05);

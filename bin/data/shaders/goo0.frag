...
vec4 texCube( sampler2DRect sam, in vec3 p, in vec3 n, in float k )
{
    vec4 x = texture2DRect( sam, p.yz );
    vec4 y = texture2DRect( sam, p.zx );
    vec4 z = texture2DRect( sam, p.xy );
...
void main( void )
{
    ...
    grow = smoothstep( 0.0, 1.0, (soundScale*5.0-vec4(0.0,1.0,2.0,3.0))/3.0 );
    ...
    float an = 1.1 + 0.05*(soundCount-10.0) - 7.0*m.x;
    ...
    col = pow( col, vec3(1.0,1.5,1.4) ) + vec3(0.0,0.02,0.14);
    
    col += (1.0/255.0)*hash1( gl_FragCoord.xy );
    
    gl_FragColor = vec4( vec3(col.g,col.r,col.b), 1.0 );
}
